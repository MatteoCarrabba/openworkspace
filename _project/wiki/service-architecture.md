...
This worktree/branch carries **Phase 3** of decision-7 — compute-plane
cleanup: `automation.toml` accepts `runs_on` as the forward name for
declared placement (legacy `machines` kept as a backward-compatible alias;
declaring both is only accepted when they agree), `on_dormant_project` now
reads a project's **effective** lifecycle (decision-2's metadata-primary
value, not folder location) closing the one remaining location-primary spot,
and `_project/wiki/compute-plane-vestigial-catalog.md` catalogs exactly
which peer-coordination machinery (heartbeats, staleness warnings,
cross-machine drift-healing, `mint-suffix`) becomes safe to cut once a hub
exists — **nothing has been removed**; every laptop automation still
depends on parts of it today.

**Phase 1 (the warm read-model + write-race guard, plus decision-8's React
client) and Phase 2 (`locations.toml` externalization) were built in sibling
worktrees/branches this same session and are not present on this branch.**
Do not assume the dashboard's warm model, `touchAndWrite` optimistic
concurrency, or `locations.toml`/`projects locations`/`projects reindex`
exist here until those branches land. Phase 4 (the hub) is explicitly
deferred, gated on token rotation (Personal OS task-201/203) — that is also
when the vestigial-catalog cuts above become live decisions, not before.
